...
resource "aws_iam_policy" "APICallpolicy" {
  name        = "FGT-${var.tag_name_prefix}-APICall_policy"
  path        = "/"
...
data "cloudinit_config" "config" {
  ...
  part {
    ...
    content      = var.license_type == "byol" ? (var.license_format == "token" ? "LICENSE-TOKEN:${chomp(file("${var.licenses[0]}"))}" : "${file("${var.licenses[0]}")}") : ""
  }
}
...
data "cloudinit_config" "config2" {
  ...
  part {
    filename     = "license"
    content_type = "text/plain"
    content      = var.license_type == "byol" ? (var.license_format == "token" ? "LICENSE-TOKEN:${chomp(file("${var.licenses[1]}"))}" : "${file("${var.licenses[1]}")}") : ""
  }
}
